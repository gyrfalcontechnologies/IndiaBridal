...
public class FetchBasicInfoForProfilesShortlistedYouCommand 
{
	final static Logger logger = Logger.getLogger(FetchBasicInfoForProfilesShortlistedYouCommand.class);

	public static void main(String args[])
	{
		FetchBasicInfoForProfilesShortlistedYouCommand searchObj = new FetchBasicInfoForProfilesShortlistedYouCommand();
	...
	public String buildQueryBasedOnPref(String memberID, int offsetVal)
	{
		...
		selClauseSB.append("SELECT ud.memberID, ud.isPremiumMember, ud.FullName, DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(), ud.DOB)), \"%Y\")+0 AS age, "
				...
				+ "WHERE ud.memberID IN (SELECT memberID FROM useractivity where shorlistedMemID = '"+memberID+"') "

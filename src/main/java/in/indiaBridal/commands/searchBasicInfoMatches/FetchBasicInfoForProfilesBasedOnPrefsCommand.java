...
public class FetchBasicInfoForProfilesBasedOnPrefsCommand 
{
	final static Logger logger = Logger.getLogger(FetchBasicInfoForProfilesBasedOnPrefsCommand.class);
	PartnerPrefDetails partPrefDetObj;
	
	public static void main(String args[])
	{
		FetchBasicInfoForProfilesBasedOnPrefsCommand searchObj = new FetchBasicInfoForProfilesBasedOnPrefsCommand();
	...
	public HashMap getResponseHashMap(String memberID, int offsetVal)
	{
		...
		if(partPrefDetObj!=null && partPrefDetObj.getStatus().equalsIgnoreCase("success"))
		{
			...
			returnMap = fetchBasicPrefInfoObj.getResponseHashMap(memberID, sqlQuery);
			if(returnMap!=null)
			{
				returnMap.put("partPrefDet", partPrefDetObj);
				logger.debug("partPrefDet object is set");
			}
			else
			{
				returnMap.put("status", "failure");
				returnMap.put("errorMsg", "failed to get basic profile details");
				returnMap.put("isnavPageDataAvailable", "false");
				logger.debug("partPrefDet object is NOT set");
			}
			return returnMap;
		}
		else
